...
class WebSoundService implements SoundService {
  ...
  Future<void> initialize() async {
    ...
    print("WebSoundService using AudioEngine.context. State: ${AudioEngine.context.state}");
  ...
  Future<void> _preload(String assetPath) async {
    ...
    try {
      ...
      final web.AudioBuffer audioBuffer = await AudioEngine.context.decodeAudioData(arrayBuffer).toDart;
  ...
  void playSfx(String assetPath) {
    ...
    if (AudioEngine.context.state == 'suspended') {
      print("AudioContext suspended, attempting to resume before playing: $assetPath");
      AudioEngine.context.resume().toDart.then((_) {
        print("AudioContext resumed successfully. State: ${AudioEngine.context.state}");
        _doPlay(assetPath);
      }).catchError((e) {
  ...
  void _doPlay(String assetPath) {
    ...
    if (buffer == null) {
      print("Sound buffer not found in cache for $assetPath. Attempting lazy load...");
      _preload(assetPath).then((_) => _doPlay(assetPath));
      return;
    }

    try {
      print("Starting playback for $assetPath (Duration: ${buffer.duration}s, Context: ${AudioEngine.context.state})");
